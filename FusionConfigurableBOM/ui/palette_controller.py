...
class PaletteController:
    ...
    def refresh(self, palette):
        ...
        try:
            config = self.store.load(design.rootComponent); self.rows, self.components = scan_design(design, [f.field_id for f in config.fields])
            self.send(palette, {'type':'state','config': self._config(config), 'table': build_table(self.rows, config.views[0])})
    ...
    def receive(self, palette, raw):
        try:
            ...
            if action == 'save_cell':
                ...
                write_value(component, message['field_id'], message.get('value',''))
            ...
            self.refresh(palette); self.send(palette, {'type':'status','message':'Saved.'})
        except Exception as exc: self.send(palette, {'type':'error','message':str(exc)})
